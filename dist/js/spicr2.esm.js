  ...
  this.slideTo = function (nextIdx) {
    ...
    timer = setTimeout(function () {
      ...
      if (isSlider) {
        ...
        // SL
        tws = spicrConnect.slides(element, slides, activeIndex, nextActive, slideDirection);
 ...
 */
function slidesTF(elem, items, active, next, direction) {
  var carouselTweens = [];
  var data = getSlideData(elem);
  var fromActive = {};
  var toActive = {};
  var fromNext = {};
  var toNext = {};
  ...
  var activeTransform = getComputedStyle(items[active]).getPropertyValue('transform');
  ...
  carouselTweens.push(spicrConnect.fromTo(elem, fromActive, toActive, optionsActive));
  // if (activeLayers) {
...
function initComponent(input) {
  ...
  Spicrs.forEach(function (x) { return new Spicr$1(x); });
}

// export to "global"
Spicr$1.initComponent = initComponent;
